...
description: 認識信頼度レベルを含みます
type: docs
weight: 13
url: /ja/androidjava/com.aspose.barcode.barcoderecognition/barcodeconfidence/
---
**Inheritance:**
java.lang.Object
```
...
| フィールド | Description |
| --- | --- |
| [MODERATE](#MODERATE) | バーコード（主に 1D バーコード）の認識信頼度（checksumm が弱い、またはなしの場合）。 |
| [NONE](#NONE) | コードテキストが正しく認識されなかった、またはバーコードが可能性のある fakeBarCodeExtendedParameters として検出された場合のバーコードの認識信頼度。 |
| [STRONG](#STRONG) | Reed\\u2013Solomon のような BCH コードで確認された認識信頼度。 |
## Methods
...
バーコード（主に 1D バーコード）の認識信頼度（checksumm が弱い、またはなしの場合）。信頼度が低い場合、コードテキストに誤認識や偽の認識が含まれる可能性があります。
...
コードテキストが正しく認識されなかった、またはバーコードが可能性のある fakeBarCodeExtendedParameters として検出された場合のバーコードの認識信頼度。
...
Reed\\u2013Solomon のような BCH コードで確認された認識信頼度。読み取ったコードテキストにエラーや偽の認識があってはなりません。
