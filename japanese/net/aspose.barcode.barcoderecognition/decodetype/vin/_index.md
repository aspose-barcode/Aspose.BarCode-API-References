---
title: VIN
second_title: Aspose.BarCode for.NETAPIリファレンス
description: でデータをデコードする必要があることを指定します ヴィン 車両識別番号バーコード仕様
type: docs
weight: 850
url: /ja/net/aspose.barcode.barcoderecognition/decodetype/vin/
---
## DecodeType.VIN field

でデータをデコードする必要があることを指定します **ヴィン** （車両識別番号）バーコード仕様

```csharp
public static readonly SingleDecodeType VIN;
```

### 関連項目

* class [SingleDecodeType](../../singledecodetype/)
* class [DecodeType](../)
* 名前空間 [Aspose.BarCode.BarCodeRecognition](../../decodetype/)
* 組み立て [Aspose.BarCode](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.BarCode.dll -->
