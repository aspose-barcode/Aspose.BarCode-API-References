---
title: TryParse
second_title: Aspose.BarCode for .NET API 参考
description: 将 SingleDecodeType 的字符串表示形式转换为其实例 返回值指示转换是成功还是失败
type: docs
weight: 40
url: /zh/net/aspose.barcode.barcoderecognition/basedecodetype/tryparse/
---
## TryParse(string, out SingleDecodeType) {#tryparse_2}

将 SingleDecodeType 的字符串表示形式转换为其实例。 返回值指示转换是成功还是失败。

```csharp
public static bool TryParse(string parsingType, out SingleDecodeType result)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| parsingType | String | 包含要转换的 SingleDecodeType 表示的字符串。 |
| result | SingleDecodeType& | 当转换成功完成时，返回一个实际的 SingleDecodeType； |

### 返回值

**true** 如果 s 转换成功；否则， **false** 。

### 也可以看看

* class [SingleDecodeType](../../singledecodetype)
* class [BaseDecodeType](../../basedecodetype)
* 命名空间 [Aspose.BarCode.BarCodeRecognition](../../basedecodetype)
* 部件 [Aspose.BarCode](../../../)

---

## TryParse(string, out MultyDecodeType) {#tryparse_1}

将 MultyDecodeType 的字符串表示形式转换为其实例。 返回值指示转换是成功还是失败。

```csharp
public static bool TryParse(string parsingType, out MultyDecodeType result)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| parsingType | String | 包含要转换的 MultyDecodeType 表示的字符串。 |
| result | MultyDecodeType& | 当转换成功完成时，返回一个实际的 MultyDecodeType； |

### 返回值

**true** 如果 s 转换成功；否则， **false** 。

### 也可以看看

* class [MultyDecodeType](../../multydecodetype)
* class [BaseDecodeType](../../basedecodetype)
* 命名空间 [Aspose.BarCode.BarCodeRecognition](../../basedecodetype)
* 部件 [Aspose.BarCode](../../../)

---

## TryParse(string, out BaseDecodeType) {#tryparse}

在确定具体类型后，将 BaseDecodeType 的字符串表示形式转换为其实例。 返回值指示转换是成功还是失败。

```csharp
public static bool TryParse(string parsingType, out BaseDecodeType result)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| parsingType | String | 包含要转换的 MultyDecodeType 表示的字符串。 |
| result | BaseDecodeType& | 当转换成功完成时，返回一个实际的 MultyDecodeType； |

### 返回值

**true** 如果 s 转换成功；否则， **false** 。

### 也可以看看

* class [BaseDecodeType](../../basedecodetype)
* 命名空间 [Aspose.BarCode.BarCodeRecognition](../../basedecodetype)
* 部件 [Aspose.BarCode](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.BarCode.dll -->
