---
title: IBarCodeGeneratorControl
second_title: Aspose.BarCode voor .NET API-referentie
description: IBarCodeGeneratorControl voor TypeConverters
type: docs
weight: 1360
url: /nl/net/aspose.barcode.windows.forms/ibarcodegeneratorcontrol/
---
## IBarCodeGeneratorControl interface

IBarCodeGeneratorControl voor TypeConverters

```csharp
public interface IBarCodeGeneratorControl
```

## Eigenschappen

| Naam | Beschrijving |
| --- | --- |
| [EncodeType](../../aspose.barcode.windows.forms/ibarcodegeneratorcontrol/encodetype/) { get; set; } | BarCode's coderingstype (symbologie). |
| [RotationAngle](../../aspose.barcode.windows.forms/ibarcodegeneratorcontrol/rotationangle/) { get; set; } | Rotatiehoek streepjescodebeeld, gemeten in graden, bijv. RotationAngle = 0 of RotationAngle = 360 betekent geen rotatie. Als RotationAngle NIET gelijk is aan 90, 180, 270 of 0, kan het voor de scanner moeilijker worden om het beeld te lezen. |

### Zie ook

* naamruimte [Aspose.BarCode.Windows.Forms](../../aspose.barcode.windows.forms/)
* montage [Aspose.BarCode](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.BarCode.dll -->
