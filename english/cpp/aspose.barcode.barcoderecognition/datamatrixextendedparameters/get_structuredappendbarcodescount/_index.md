---
title:  method
linktitle: get_StructuredAppendBarcodesCount
second_title: Aspose.BarCode for C++ API Reference
description: ' method. Gets the DataMatrix structured append mode barcodes count. Default value is -1. Count must be a value from 1 to 35 in C++.'
type: docs
weight: 100
url: /cpp/aspose.barcode.barcoderecognition/datamatrixextendedparameters/get_structuredappendbarcodescount/
---
## DataMatrixExtendedParameters::get_StructuredAppendBarcodesCount method


Gets the [DataMatrix](../../../aspose.barcode.barcoderecognition.datamatrix/) structured append mode barcodes count. Default value is -1. Count must be a value from 1 to 35.

```cpp
int32_t Aspose::BarCode::BarCodeRecognition::DataMatrixExtendedParameters::get_StructuredAppendBarcodesCount() const
```

## Remarks


The count of the [DataMatrix](../../../aspose.barcode.barcoderecognition.datamatrix/) structured append mode barcode.
## See Also

* Class [DataMatrixExtendedParameters](../)
* Namespace [Aspose::BarCode::BarCodeRecognition](../../)
* Library [Aspose.BarCode for C++](../../../)
