...
|add_or_replace(node)|  |
|add_or_replace(id, value)|Adds a new DataElement with the specified identifier and value, <br/>            or replaces the existing element if an entry with the same ElementID is already present.|
|find_data_element(id, is_open_or_create)|Searches for data element by 3-letter id|
|clear()|Clears all data elements|
|insert(index, node)|Inserts the specified DataElement at the given index. <br/>            If a DataElement with the same ElementID already exists, it will be replaced.|
