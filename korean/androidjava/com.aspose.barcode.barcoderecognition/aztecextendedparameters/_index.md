...
description: Aztec 인식 바코드의 특수 데이터를 저장합니다
type: docs
weight: 12
url: /ko/androidjava/com.aspose.barcode.barcoderecognition/aztecextendedparameters/
---
**Inheritance:**
...
| Method | 설명 |
| --- | --- |
| [equals(Object obj)](#equals-java.lang.Object-) | 이 인스턴스가 지정된  AztecExtendedParameters  값과 같은지 여부를 나타내는 값을 반환합니다. |
| [getClass()](#getClass--) |  |
| [getStructuredAppendBarcodeId()](#getStructuredAppendBarcodeId--) | Aztec 구조 추가 모드 바코드의 ID를 가져옵니다. |
| [getStructuredAppendBarcodesCount()](#getStructuredAppendBarcodesCount--) | Aztec 구조 추가 모드 바코드 수를 가져옵니다. |
| [getStructuredAppendFileId()](#getStructuredAppendFileId--) | Aztec 구조 추가 모드의 파일 ID를 가져옵니다. |
...
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) | 이  AztecExtendedParameters  의 사람이 읽을 수 있는 문자열 표현을 반환합니다. |
| [wait()](#wait--) |  |
...
이 인스턴스가 지정된  AztecExtendedParameters  값과 같은지 여부를 나타내는 값을 반환합니다.
...
Aztec 구조 추가 모드 바코드의 ID를 가져옵니다. ID는 1부터 시작하며 바코드 수보다 작거나 같아야 합니다. 기본값은 0입니다.

값: Aztec 구조 추가 모드의 바코드 ID.
...
Aztec 구조 추가 모드 바코드 수를 가져옵니다. 기본값은 0이며, 카운트는 1에서 26 사이의 값이어야 합니다.

값: Aztec 구조 추가 모드의 바코드 수.
...
Aztec 구조 추가 모드의 파일 ID를 가져옵니다. 기본값은 빈 문자열입니다.

값: Aztec 구조 추가 모드의 파일 ID.
...
이  AztecExtendedParameters  의 사람이 읽을 수 있는 문자열 표현을 반환합니다.

**Returns:**
java.lang.String - 이  AztecExtendedParameters 를 나타내는 문자열입니다.
### wait() {#wait--}
```
