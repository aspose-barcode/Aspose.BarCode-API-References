...
description: DataMatrix 인식 바코드의 특수 데이터를 저장합니다
type: docs
weight: 31
url: /ko/androidjava/com.aspose.barcode.barcoderecognition/datamatrixextendedparameters/
---
**Inheritance:**
...
| Method | 설명 |
| --- | --- |
| [equals(Object obj)](#equals-java.lang.Object-) | 이 인스턴스가 지정된 DataMatrixExtendedParameters 값과 같은지 여부를 나타내는 값을 반환합니다 |
| [getClass()](#getClass--) |  |
| [getStructuredAppendBarcodeId()](#getStructuredAppendBarcodeId--) | DataMatrix 구조 추가 모드 바코드의 ID를 가져옵니다 |
| [getStructuredAppendBarcodesCount()](#getStructuredAppendBarcodesCount--) | DataMatrix 구조화 추가 모드 바코드 수를 가져옵니다. |
| [getStructuredAppendFileId()](#getStructuredAppendFileId--) | DataMatrix 구조 추가 모드 바코드의 ID를 가져옵니다 |
| [hashCode()](#hashCode--) | 이 인스턴스의 해시 코드를 반환합니다. |
| [isEmpty()](#isEmpty--) | 모든 매개변수가 기본값만 가지고 있는지 테스트합니다. |
| [isReaderProgramming()](#isReaderProgramming--) | 코드가 바코드 리더에게 초기화 또는 재프로그래밍 지시로 다음 데이터를 해석하도록 사용되는지 여부를 나타냅니다. |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [op_Equality(DataMatrixExtendedParameters first, DataMatrixExtendedParameters second)](#op-Equality-com.aspose.barcode.barcoderecognition.DataMatrixExtendedParameters-com.aspose.barcode.barcoderecognition.DataMatrixExtendedParameters-) | 첫 번째 DataMatrixExtendedParameters 값이 두 번째와 같은지 여부를 나타내는 값을 반환합니다. |
| [op_Inequality(DataMatrixExtendedParameters first, DataMatrixExtendedParameters second)](#op-Inequality-com.aspose.barcode.barcoderecognition.DataMatrixExtendedParameters-com.aspose.barcode.barcoderecognition.DataMatrixExtendedParameters-) | 첫 번째 DataMatrixExtendedParameters 값이 두 번째와 다른지 여부를 나타내는 값을 반환합니다. |
| [toString()](#toString--) | 이 DataMatrixExtendedParameters에 대한 사람이 읽을 수 있는 문자열 표현을 반환합니다. |
| [wait()](#wait--) |  |
...
이 인스턴스가 지정된 DataMatrixExtendedParameters 값과 같은지 여부를 나타내는 값을 반환합니다
...
DataMatrix 구조화 추가 모드 바코드의 ID를 가져옵니다. ID는 1부터 시작하며 바코드 수보다 작거나 같아야 합니다. 기본값은 -1입니다.

값: DataMatrix 구조화 추가 모드 바코드의 ID.
...
DataMatrix 구조화 추가 모드 바코드 수를 가져옵니다. 기본값은 -1이며, 카운트는 1에서 35 사이의 값이어야 합니다.

값: DataMatrix 구조화 추가 모드 바코드의 수.
...
첫 번째 DataMatrixExtendedParameters 값이 두 번째와 같은지 여부를 나타내는 값을 반환합니다.

**Parameters:**
| Parameter | Type | 설명 |
| --- | --- | --- |
| first | [DataMatrixExtendedParameters](../../com.aspose.barcode.barcoderecognition/datamatrixextendedparameters) | 첫 번째 비교 값 |
| second | [DataMatrixExtendedParameters](../../com.aspose.barcode.barcoderecognition/datamatrixextendedparameters) | 두 번째 비교 값 |
...
첫 번째 DataMatrixExtendedParameters 값이 두 번째와 다른지 여부를 나타내는 값을 반환합니다.
...
이 DataMatrixExtendedParameters에 대한 사람이 읽을 수 있는 문자열 표현을 반환합니다.

**Returns:**
`java.lang.String` - 이 DataMatrixExtendedParameters를 나타내는 문자열입니다.
### wait() {#wait--}
```
